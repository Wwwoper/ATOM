...
Этот модуль предоставляет функциональность для управления процессом доставки посылок,
включая расчет стоимости, валидацию данных и обработку финансовых операций.

Основные компоненты:
    - PackageDeliveryService: Основной класс для управления доставкой
    - Методы расчета стоимости доставки
    - Валидация данных доставки
    - Подготовка данных для транзакций
    - Управление статусом оплаты

Процесс обработки доставки:
    1. Валидация данных посылки и доставки
    2. Расчет стоимости доставки в рублях
    3. Подготовка данных для финансовых операций
    4. Обработка оплаты или возврата
    5. Обновление статуса доставки

Финансовые операции:
    - Расчет стоимости доставки с учетом курса обмена
    - Подготовка данных для транзакций оплаты/возврата
    - Обработка статуса оплаты
    - Сброс финансовых данных при необходимости

Примеры использования:
    service = PackageDeliveryService()
    
    # Расчет стоимости доставки
    service.calculate_delivery_costs(delivery)
    
    # Подготовка данных для транзакции
    transaction_data = service.serialize_delivery_data_for_transaction(delivery)

Примечания:
    - Все финансовые расчеты выполняются с учетом курса обмена
    - Реализована защита от повторной оплаты
    - Поддерживается система валидации данных
    - Ведется учет времени оплаты
"""
...
class PackageDeliveryService:
    ...
    def _validate_delivery(self, delivery) -> None:
        """
        Валидация доставки для создания транзакции.
